...
function indekseGoreFenomen(fenomenDizisi, index) {
  for (let i = 0; i < fenomenDizisi.length; i++) {
    if (i = index) {
      return `${index}. indekste bulunan fenomen: ${fenomenDizisi[i].profile}`
    }
  }
}
...
function fenomenEkle(fenomenDizisi, no, isim, takipci, gonderi, uygulama) {
  const yeniDizi = [...fenomenDizisi];
  const yeniNesne = {
    "number": no,
    "profile": isim,
...
function enFenomenler(fenomenDizisi) {
  const yeniDizi = [];
  for (let i = 0; i < fenomenDizisi.length; i++) {
...
function fenomenGonderimSayisi(fenomenDizisi, isim){
  for (let i = 0; i < fenomenDizisi.length; i++) {
    if (fenomenDizisi[i].profile === isim) {
      return fenomenDizisi[i].posts;
    }
  };
}
...
function platformaGoreCokGonderiYapanFenomen(fenomenDizisi, uygulama){
  let gonderiSayisi = 0;
  let kisi = ""
  for (let i = 0; i < fenomenDizisi.length; i++) {
    if (fenomenDizisi[i].platform === uygulama && gonderiSayisi < fenomenDizisi[i].posts) {
      gonderiSayisi = fenomenDizisi[i].posts;
      kisi = fenomenDizisi[i].profile;
    }
  }
  return kisi;
}
